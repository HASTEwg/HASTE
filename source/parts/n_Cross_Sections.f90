...
Module n_Cross_Sections
    ...
    Type :: res_sig_Type
        !HACK Resonance CS is included as linearly interpolable listing rather than by actual resonance reconstruction, this 
        !violates the small-memory intent of this cross sections implementation
        ...
        Real(dp), Allocatable :: sig(:,:)  ![barns] has dimension 1:n_E abd 1:2, dim 2 is 1=n-gamma resonance and 2=elastic scatter
                                           ! resonance
    End Type
    ...
    Type :: CS_Type
        ...
        Logical, Allocatable :: has_res_cs(:)  !has dimension 1:n_iso, TRUE indicates resonance cross sections are included in an 
                                               !isotope's cross section representation
    ...
    Real(dp), Parameter :: k_B = k_Boltzmann / 1000._dp**2  ![J/K]*[1 km^2 / 1000^2 m^2] Boltzmann const with convenient units
...
Function Setup_Cross_Sections(resources_directory,cs_setup_file,elastic_only,aniso_dist,E_min,E_max) Result(CS)
    ...
    Do i = 1,CS%n_iso
        ...
        If (stat .NE. 0) Call Output_Message( 'ERROR:  Cross_Sections: Setup_Cross_Sections:  File open error, '// & 
                                            & cs_file_name//', IOSTAT=',stat,kill=.TRUE.)
    ...
    If (n_energies .GT. Huge(CS%n_E_uni)) Call Output_Message('ERROR:  Cross_Sections: Setup_Cross_Sections:  Length of unified & 
                                                             &energy grid exceeds available index',kill=.TRUE.)
    ...
    !Now read each file (yes, again) into its appropriate place in the cross section structure, constructing the integer maps and 
    !keys as we go
    CS%n_a_max = 0
    CS%n_a_tab_max = 0
    Do i = 1,CS%n_iso
        ...
        Do j = 1,n_absorption_modes
            ...
            If (Trim_CS_for_E(n_p,E_scratch,CS_scratch,n_r,Interp_scratch,E_min,E_max)) Then
                Call Map_and_Store_CS( CS%n_E_uni,CS%E_uni,n_p,E_scratch,CS_scratch,n_r,Interp_Scratch,CS%abs_cs(i)%sig(j), & 
        ...
        ! If (.NOT. elastic_only) Then  !need inelastic level cross section files
            ...
            Do j = 1,n_inelastic_lev
                ...
                If (Trim_CS_for_E(n_p,E_scratch,CS_scratch,n_r,Interp_scratch,E_min,E_max)) Then
                    ...
                    Call Map_and_Store_CS( CS%n_E_uni,CS%E_uni,n_p,E_scratch,CS_scratch,n_r,Interp_Scratch,CS%lev_cs(i)%sig(j), & 
                ...
                If (aniso_dist) Then  !need inelastic level ang dist files
                    ...
                    Call Read_Ang_Dist_file(cs_file_name,E_scratch,Ang_dist_scratch,n_p,ltt)
                    If (Trim_AD_for_E(n_p,E_scratch,Ang_dist_scratch,E_min,E_max)) Then
                        Call Map_and_Store_AD( CS%n_E_uni,CS%E_uni,n_p,E_scratch,Ang_dist_scratch,CS%lev_cs(i)%da(j), & 
                                             & CS%lev_cs(i)%thresh(j))
                    End If
                    Deallocate(E_scratch,Ang_dist_scratch)
                    If (ltt .EQ. 1) Then
                    ...
                    Else If (ltt .EQ. 2) Then
                        If (MaxVal(CS%lev_cs(i)%da(j)%da(:)%n_a) .GT. CS%n_a_tab_max)  CS%n_a_tab_max = &
                                                                                     & MaxVal(CS%lev_cs(i)%da(j)%da(:)%n_a)
                    Else If (ltt .EQ. 3) Then
    ...
    j = 1
    Do i = 1,n_elements
        CS%iso_fractions(j:Sum(n_isotopes(1:i))) = el_fractions(i) * CS%iso_fractions(j:Sum(n_isotopes(1:i))) / &
                                                 & Sum( CS%iso_fractions(j:Sum(n_isotopes(1:i))) )
...
Subroutine Read_CS_file(CS_file_name,Q,An,E_list,CS_list,Int_list,n_p,n_r,just_n)
    ...
    If (stat .NE. 0) Call Output_Message( 'ERROR:  Cross_Sections: Read_CS_file:  File open error, '// & 
                                        & CS_file_name//', IOSTAT=',stat,kill=.TRUE.)
    !the first line's second entry is the mass of the target in neutron masses
    Read(cs_unit,'(2E11.6E1)') trash, An
    !the next line's second entry is the Q-value for the reaction, the fifth and sixth entries are the number of interpolation 
    !ranges and energy levels in the file
    Read(cs_unit,'(4E11.6E1,2I11)') trash, Q, trash, trash, n_r, n_p
    If (n_r .GT. 3) Call Output_Message( 'ERROR:  Cross_Sections: Read_CS_file:  Number of interpolation ranges greater than 3: ' &
                                       & //CS_file_name//', n_r=',n_r,kill=.TRUE.)
    ...
    End If
    If (Any(Int_list(:,2).GT.5) .OR. Any(Int_list(:,2).LT.1)) Call Output_Message('ERROR:  Cross_Sections: Read_CS_file:  Unknown &
                                                                                 &interpolation scheme: '//CS_file_name,kill=.TRUE.)
    !Allocate lists
    ...
    Q = Abs(Q) / 1000._dp  !also store Q as positive value, negativity of Q for inelastic scatter is handled by energy book-keeeping
                           !formulations during transport
...
Subroutine Read_Ang_Dist_file(Coeff_file_name,E_list,da_list,n_p,LTT,just_n)
    ...
    If (stat .NE. 0) Call Output_Message( 'ERROR:  Cross_Sections: Read_Ang_Dist_file:  File open error, '// & 
                                        & Coeff_file_name//', IOSTAT=',stat,kill=.TRUE.)
    ...
    If (LCT .NE. 2) Call Output_Message( 'ERROR:  Cross_Sections: Read_Ang_Dist_file:  Incorrectly formatted file, '// & 
                                       & Coeff_file_name//', LCT=',LCT,kill=.TRUE.)
    !Skip the next line
    Read (coeff_unit,*)
...
Function sig_S(CS,E,iE_get,iE_put)
    ...
    sig_S = 0._dp
    Do i = 1,CS%n_iso
        sig_S = sig_S + CS%iso_Fractions(i) * sig_Composite( E,CS%n_E_uni,CS%E_uni,CS%lnE_uni,E_index,0,CS%lev_cs(i)%n_lev, & 
                                                           & CS%lev_cs(i)%thresh,CS%lev_cs(i)%sig)
        If (CS%has_res_cs(i)) Then
            If (E.GT.CS%res_cs(i)%E_range(1) .AND. E.LT.CS%res_cs(i)%E_range(2)) Then
                sig_S = sig_S + CS%iso_Fractions(i) * sig_res(E,CS%res_cs(i)%n_E,CS%res_cs(i)%E,CS%res_cs(i)%sig(:,2))
            End If
        End If
    End Do
    If (Present(iE_get)) iE_get = E_index
...
Function sig_S_iso(CS,iso,E,iE_get,iE_put)
    ...
    Else
        E_index = Bisection_Search(E,CS%E_uni,CS%n_E_uni)
    End If
    sig_S_iso = sig_Composite( E,CS%n_E_uni,CS%E_uni,CS%lnE_uni,E_index,0,CS%lev_cs(iso)%n_lev,CS%lev_cs(iso)%thresh, & 
                             & CS%lev_cs(iso)%sig)
    If (CS%has_res_cs(iso)) Then
        If (E.GT.CS%res_cs(iso)%E_range(1) .AND. E.LT.CS%res_cs(iso)%E_range(2)) Then
            sig_S_iso = sig_S_iso + sig_res(E,CS%res_cs(iso)%n_E,CS%res_cs(iso)%E,CS%res_cs(iso)%sig(:,2))
        End If
    End If
    If (Present(iE_get)) iE_get = E_index
...
Function sig_A(CS,E,iE_get,iE_put)
    ...
    sig_A = 0._dp
    Do i = 1,CS%n_iso
        sig_A = sig_A + CS%iso_Fractions(i) * sig_Composite( E,CS%n_E_uni,CS%E_uni,CS%lnE_uni,E_index,1,CS%abs_cs(i)%n_modes, & 
                                                           & CS%abs_cs(i)%thresh,CS%abs_cs(i)%sig)
        If (CS%has_res_cs(i)) Then
            If (E.LT.CS%res_cs(i)%E_range(2)) Then 
                sig_A = sig_A + CS%iso_Fractions(i) * sig_res(E,CS%res_cs(i)%n_E,CS%res_cs(i)%E,CS%res_cs(i)%sig(:,1))
            End If
        End If
    End Do
    If (Present(iE_get)) iE_get = E_index
...
Subroutine Broad_sig_start(E,M,T,v,gamma,vRmin,vRmax)
    ...
    If (vT .LE. v) Then
        vRmin = v - vT
    Else
        !HACK Use small VRmin instead of zero to prevent Log(E=0) from appearing in log interpolation cases... 
        !1.E-16 equates to an energy of about 1.E-38 keV
...
Recursive Subroutine Continue_Broad_Romberg_T_A(CS,iE,vR1,vR2,gamma,v,s1,s2,d,R0,level,sig_T_A)
    ...
    If (level .GT. 5) Then !max allowed recursion depth, interval has been split 50 times...
        Call Output_Message('ERROR:  Cross_Sections: Continue_Broad_Romberg_T_A:  Failed to converge before reaching max recursion &
                           &depth.',kill=.TRUE.)
    End If
    !if we get this far, we failed to converge
...
Recursive Subroutine Continue_Broad_Romberg_T(CS,iE,vR1,vR2,gamma,v,s1,s2,d,R0,level,sigT)
    ...
    End Do
    If (level .GT. 5) Then !max allowed recursion depth, interval has been split 50 times...
        Call Output_Message('ERROR:  Cross_Sections: Continue_Broad_Romberg_T:  Failed to converge before reaching max recursion &
                           &depth.',kill=.TRUE.)
    End If
    !if we get this far, we failed to converge
...
Recursive Subroutine Continue_Broad_Romberg_S_iso(CS,iso,iE,vR1,vR2,gamma,v,s1,s2,d,R0,level,sigS)
    ...
    End Do
    If (level .GT. 5) Then !max allowed recursion depth, interval has been split 50 times...
        Call Output_Message('ERROR:  Cross_Sections: Continue_Broad_Romberg_S_iso:  Failed to converge before reaching max &
                           &recursion depth.',kill=.TRUE.)
    End If
    !if we get this far, we failed to converge
